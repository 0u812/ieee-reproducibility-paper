...
Non-determinism in software usually comes from two main sources:
Concurrency and/or asynchronous execution, and
 memory layout.
...
% publications.
The \textit{M. genitalium} whole-cell model contains a wide variety of modeling
methods, and makes use of these methods in a highly specific way which may
not be immediately intuitive or easy to understand in terms of rationale.
Hence, it highlights the need for summarizing the
dynamical processes used by the model without requiring the user to be an expert on the model's
source code.
...
Such data should be systematically annotated in order to ensure that
...
factor which should be incorporated into standards and their respective
...
A separate integration module can then update shared variables deterministically,
thus achieving replicability.
