...
\textit{Conclusion:} 
We concluded that new standards and simulation software tools are needed to enhance and verify the reproducibility of systems biology modeling. 
...
There are three requirements for reproducible systems biology modeling (Fig.~\ref{fig_repro_diagram}). (1) Researchers must record every data source and assumption used to construct models so that other researchers can generate the same species, reactions, and rate laws. (2) Researchers must record every algorithm and parameter value used to simulate models so that other researchers can generate the same numerical results. (3) Researchers must ensure that every simulation software tool produces statistically indistinguishable results so that researchers who use different software programs will reach the same conclusions.

\section{The Path to Reproducible Modeling}

Several new tools and practices are needed to help researchers reproducibly build and simulate systems biology models. (1) More comprehensive experimental databases must be developed to provide the data needed to build systems models.  For example, Karr et al. developed the WholeCellKB database \cite{karr2013wholecellkb} to organize the over 1,400 quantitative measurements used to build the \textit{M. genitalium} WC model. These databases should be machine-readable so that they can be automatically queried and used to build models. These databases should also use data models similar to that of predictive models to clarify how they can be used to build models. (2) New model design tools are needed to record how models are built from experimental data and record the design choices and assumptions used to construct models. The result of these tools should be unambiguous descriptions of the modeled biology, including cross references to external databases, and clear descriptions of how models are built from experimental data. Ontologies should be developed and used to describe model assumptions. (3) Researchers should use standards to describe models and simulations or share source code so that other researchers know exactly what algorithms and configuration options were used to generate simulation results. (4) Researchers should systematically verify the repeatability of every simulation software program and computational model.
...
\subsection{Special Considerations for Parallel Simulation Software}

Multi-threaded and multi-core programs execute individual instructions in a non-deterministic order that is determined by the program's operating environment. Multi-threaded and multi-core programs also non-deterministically allocate memory in ways that are determined by their operating environment. Special care must be taken to ensure that parallel simulation software tools are repeatable despite non-deterministic thread execution and memory layout.
...
\begin{figure*}[tb!]
  ...
  \end{subfigure}
  \caption{Mean and variance testing is not an effective way to verify the reproducibility of stochastic models because these statistical metrics do not capture the behavior of all stochastic systems. For example, the bistable stochastic model described by the \karrcomment{Antimony}{It would be better to describe the model rather than the Antimony code which readers won't be familiar with} code in (a) and simulated trajectories plotted in (b) cannot be uniquely described by its mean and variance.}
  \label{fig_bistable_plot}
\end{figure*}

Computer scientists have developed several systems to deterministically run multi-threaded programs. These include C{\small ORE}D{\small ET} \cite{bergan2010coredet} and D{\small THREADS} \cite{liu2011dthreads}.
C{\small ORE}D{\small ET} is a framework and library for compiling C/C++ programs such that they behave deterministically.
...
In general, it is difficult to verify repeatability of complex stochastic models by testing their statistical equivalence across multiple simulators. Instead, we recommend that modelers and simulation software developers should (1) utilize the methods described in the previous section to deterministically simulate stochastic models and (2) verify that multiple simulation software programs generate the same numerical results.
...
\subsection{Special Considerations for Multi-algorithm Models}
Multi-algorithm models are composed of multiple submodels, each represented using different mathematics and each trained using different experimental data. This multi-algorithm methodology is motivated by the desire to model biological systems as completely as possible given our current limited data and knowledge by simultaneously using fine-grained representations such as ODEs and coarse-grained representations such as Boolean networks to represent well- and poorly-characterized pathways. We anticipate that multi-algorithm modeling will gain popularity as more researchers engage in WC modeling.

Multi-algorithm models must be simulated by concurrently integrating multiple submodels that share state. Researchers are exploring several potential methods to coordinate  submodels that share state and rigorously simulate multi-algorithm models, including shared memory buffers and message passing. The behavior of complex hybrid systems is largely unknown. Thus, these algorithms will need to be extensively tested to verify their repeatability, as well as to understand their fundamental properties such as how their runtime scales with the number of submodels and how the variance in their predicted dynamics varies with the simulation timestep.

\section{Conclusion}

Expanded standards and new software tools are needed to facilitate repeatable and reproducible systems biology modeling.
These standards should enable researchers to describe every model data source and assumption. One key step toward repeating systems biology simulation results is expanded support among SBML-compatible simulators for the 
...
Each simulator should also deterministically use pseudo-random numbers.
These new standards and expanded software tools will make it easier for researchers to understand, reproduce, and validate systems biology models. 
