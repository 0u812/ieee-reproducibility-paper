...
\textit{Results:} 
We determined that reproducible systems biology modeling requires both reproducible model building and reproducible numerical simulation.
\textit{Conclusion:} 
We also determined that new standards and simulation software tools are needed to enhance the reproducibility of systems biology modeling. 
New standards are needed to explicitly document all model assumptions. New distributed simulation tools are needed to deterministically simulate computationally expensive models.
\textit{Significance:}
...
\caption{Comparison of repeatability and reproducibility. A model is repeatable if its simulation results can be numerically replicated. A model is reproducible if the model is repeatable and the model can be replicated from its underlying scientific data and knowledge. Reproducibility is a stricter standard than repeatability.}
\label{fig_repro_diagram}
\end{figure*}

\section{\karrcomment{Repeatability and Reproducibility}{This section needs more introduction to tie all of the ideas together and to connect them to the introduction.}}

Fig.~\ref{fig_repro_diagram} \karrcomment{shows the steps involved in reproducing a model. A subset of these steps involve repeatability only. To summarize these steps, the model's equations and parameter values must be duplicated from the original assumptions and experimental data.}{These sentences need more explanation. Keep in mind readers that are not familiar with systems biology or whole-cell modeling. This needs to be readily understandable by the average electrical engineer.} This requires new machine-readable experimental databases which contain the data needed to build models, new model annotation standards for describing how these databases are used to build models, and new model annotation standards for describing the assumptions that models are based on \cite{boulton2012open}.
