...
\begin{figure*}[tb!]
  ...
  \end{subfigure}
  \caption{Mean and variance testing is not an effective way to verify the repeatability of stochastic models because these statistical metrics do not capture the behavior of all stochastic systems. For example, the bistable stochastic model described by the \karrcomment{Antimony}{It would be better to describe the model rather than the Antimony code which readers won't be familiar with} code in (a) and \karrcomment{simulated trajectories}{We should provide the code to regenerate this as a supplementary file} plotted in (b) cannot be completely specified by its mean and variance. Statistical equivalence should be tested using a multivariate Kolmogorov-Smirnov test.}
...
After adopting the methods outlined above to repeatably simulate models, we recommend that systems biology modelers and software developers verify repeatability by testing the statistical equivalence of simulation results generated by multiple simulation software programs for the same model, and then correcting any errors identified through this analysis. Bergmann and Sauro \cite{bergmann2008comparing} and Evans et al. \cite{evans2008sbml} have used this approach to assess the repeatability of models across several simulators, identifying errors in several simulation programs.
