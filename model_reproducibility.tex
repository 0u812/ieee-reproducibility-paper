...
To illustrate the difference between these two terms, consider a Gedanken experiment where Alice and Bob are both independently and separately trying to model pathway X which has been implicated in cancer. Suppose that the interactions that govern the pathway are well known and that binding affinities and Hill coefficients can be estimated from experimental data. Since Alice and Bob are both trying to model the same pathway with the same interactions, they should qualitatively arrive at the same results, assuming prior knowledge of the pathway is accurate. Upon comparing results, Alice and Bob find that their results show qualitative agreement. This is good, since independent researchers converging at the same result suggests that each employed valid reasoning and methods.

This is the most \textit{reproducible} scenario. In reality, researchers often do not converge by happenstance. For example, if Alice publishes a landmark result, Bob will liekly try to reproduce it by following Alice's published methods. This is perfectly acceptable and is a necessary part of the scientific method, but ``clean room'' scenarios like the one above vindicate reproducibility at least as much as goal-directed research designed to test a landmark result, as they eliminate potential sources of bias inherent in Alice or Bob's agenda.

On the other hand, suppose Alice and Bob's results differed
numerical discrepancies, which, in this case, they attribute to their using different parameter fitting schemes. To determine if the only discrepancy is due to different parameterization, Alice asks Bob for his parameter values in order to test them in her own model.
