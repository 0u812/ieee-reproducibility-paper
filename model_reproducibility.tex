...
For a simulation result such as a published figure to be replicable, it must be possible to regenerate the result, 
even if the simulation is run on a different machine.
Consequently, simulations can only be replicated if that are implemented using deterministic numerical solvers and 
seed-able machine-independent pseudorandom processes.
...
Despite these advances, most computational biology results still cannot easily be reproduced \cite{garijo2013quantifying}.
...
Third, the model is difficult to reproduce bcause details of how the \textit{M. genitalium} model was parameterized with 
...
because they are intertwined with the simulator source code. \karrcomment{Going foward, modelers should use transparent 
...
with the simulator source code. \karrcomment{A new standard must be developed to allow modeleres to annotate model assumptions.}{elaborate}
