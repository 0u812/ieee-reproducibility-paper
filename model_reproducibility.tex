...
and we discuss later how to improve the link between data and the processes in models which consume it.
...
model composition combine models by flattened them into a single larger model.
...
The use of these features does not prevent the model from being encoded in SBML. However, if the model w    ere encoded in SBML,
...
on the state of reproducibility and replicability in systems biology.
