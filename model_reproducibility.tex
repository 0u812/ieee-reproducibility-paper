...
Second, the model's numerical integrator must be duplicated. This requires that simulation software programs be available open-source \cite{easterbrook2014open}.

Third, the second numerical integrator must be used to numerically duplicate the model's numerical results \cite{easterbrook2014open}. In the case of stochastic models, this means that the first moments of the model's predictions must be duplicated. In turn, to duplicate numerical results simulators should have complete deterministic control over their execution, including using deterministic algorithms and deterministic pseudorandom number generators. Chaotic models, which are highly sensitive to small numerical differences, and stochastic simulators, which cannot duplicate numerical results, are considerably more difficult to reproduce.
...
modeling formalisms, SBML Level 3 includes several extensions including the
...
A third barrier to reproducibility is the lack of transparency into the experimental data and assumptions which models are based on. 
...
Threading and asynchronous input/output callbacks execute instructions in an apparently
...
on top of apparently non-deterministic multithreaded systems. Two systems that have been developed to eliminate non-determinism in threading
...
such that the generated machine code operates deterministically.
...
to achieve replicable simulation algorithms, even on top of non-deterministic platforms.
...
although the model could be encoded in SBML using the Flux Balance Constraints, Hierarchical Model Composition, Multistate and Multicomponent 
