...
Currently, individual modelers are respectively responsible for reproducibly conducting their own research. This places a substantial burden on individual researchers to make every step of their research reproducible. Alternatively, we recommend that the systems biology community develops several software tools, databases, and standards to ensure that each researcher conducts reproducible research.

(1) More comprehensive experimental databases should be developed to provide the data needed to build systems models.  For example, Karr et al. developed the WholeCellKB database \cite{karr2013wholecellkb} to organize the over 1,400 quantitative measurements used to build the \textit{M. genitalium} WC model. These databases should be machine-readable so that they can be automatically queried and used to build models. These databases should also use data models similar to that of systems biology models to clarify the connection between models and their underlying data. (2) New model design tools and Systems Biology Ontology (SBO) \cite{juty2013systems} terms are needed to record how models are built, including every design choice, assumption, and data source. Currently, researchers can use SBML annotations and SBO terms to record several common assumptions, such as the rapid equilibrium between free enzymes and enzyme-substrate complexes in Michaelis-Menten kinetics. However, many researchers do not utilize these annotations and the SBO does not represent all possible assumptions. Thus, researchers should develop model design tools which automatically record assumptions and data sources by adopting provenance tracking techniques from bioinformatics \cite{callahan2006vistrails, hillman2012using, oinn2004taverna}. The SBO should also be expanded to represent more assumptions. (3) The existing model description formats should be expanded to represent all systems biology models, including WC models. For example, to describe WC models, SBML should be expanded to support genomic sequence data and sequence-based reaction patterns. Existing standards such as SBOL \cite{galdzicki2014synthetic} are well-suited to representing sequence data and could in principle be used for this purpose. However, SBOL is used primarily for storing and exchanging genetic constructs and sequencing data. It was not originally developed for use in systems biology simulations. Whether or not to use SBOL for this purpose requires a careful analysis of the trade-offs of using it. The advantages of SBOL include the fact that (1) it is a mature standard and has been validated for the purpose of encoding sequence data, and (2) it makes use of linked data, allowing models to point to genetic constructs in a machine-readable way. Its disadvantages include (1) the lack of mature, portable, cross-language library support (in contrast to SBML) and (2) it requires simulation software developers to understand and implement support for a standard which has a much greater scope than what is required to effectively simulate WC models. More work is required in order to show that SBOL is an ideal format for encoding sequence information in WC models. Pending this, we believe that linking SBML and SBOL via an SBML Level 3 package and using the COMBINE archive to package both SBOL and SBML would be a promising route for implementation. (4) Where possible, researchers should use standard formats rather than proprietary codes to describe models and simulation experiments. For example, researchers should use SED-ML to represent every software configuration option used to generate simulation results. If a new simulation algorithm or implementation is developed that cannot be fully described within the existing framework of KISAO, researchers should notify the maintainers so that the ontology may be expanded to include new terms. Furthermore, resources such as KISAO require long-term funding in order to remain freely available. (5) The existing standards-compliant simulation software programs should be expanded to support a wider variety of models. In particular, simulation software programs should be expanded to support the Arrays, Distributions, Flux Balance Constraints, Hierarchical Model Composition, and Multistate and Multicomponent Species SBML packages. Several simulation software programs, including BioUML \cite{Kolpakov2006} and iBioSim \cite{Stevens2013}, have already begun to support these packages. Unfortunately, most simulation software developers have insufficient funding to implement every package. (6) Modeling workflows should be expanded to systematically verify the statistical repeatability of simulation results. (7) New model verification tools should be developed to automatically identify errors in models. We developed a suite of tests to systematically error-check our \textit{M. genitalium} WC model \footnote{https://github.com/CovertLab/WholeCell/tree/master/src\_test}. These tests checked for simple problems such as undefined species, reaction mass and charge imbalance, and inconsistent reaction rates among submodels. Nevertheless, we found this test suite invaluable for debugging our model. These tests should be generalized and new software tools should be developed to help researchers systematically evaluate such tests. (8) Every model, simulation experiment, simulation result, and simulation software program should be published open-source \cite{easterbrook2014open}. Here, we use the Open Source Initiative (OSI) definition of ``open-source'' \footnote{https://opensource.org/osd}. Open-source software is not necessarily reproducible. In particular, it can fail to be reproducible through poor documentation, idiosyncrasies of a particular development setup, or lack of modular design. However, the open-source model encourages critique of source code and documentation quality, which helps drive improvements in these areas, ultimately benefiting reproducibility. Lastly, journals should require authors to permanently archive every data source and model described in their publications.

\subsection{Special Considerations for Stochastic Simulation}
Stochastic simulations typically make use of pseudo-random number generators (PRNGs) which deterministically produce a certain sequence of numbers when seeded with the same initial state. Therefore, we recommend that stochastic simulation software developers provide ways to set and record PRNG seeds so that modelers can repeat simulation results. This would enable researchers to repeat not only statistical distributions, but exact trajectories, which is invaluable for identifying and debugging errors in complex models and simulation algorithms.

Furthermore, different implementations of the same PRNG often differ in subtle ways. For example, the seed methods of the Boost C++ Library \cite{schaling2011boost} and Python Standard Library implementations of the Mersenne-Twister algorithm differentially initialize the PRNGs' internal states. To date, there is no standard, unbiguous way to completely specify the PRNG algorithm used by a software program. However, we show in Supplementary Material S2 that it is possible to achieve convergence for the results of a PRNG algorithm for different codebases. In order to achieve convergence, both implementations must (1) use the same algorithm (i.e. refer to the same publication), (2) have the same parameterization of the PRNG, and (3) start in the same initial state. In our experiment described in S2, both implementations agreed on (1) and (2) but diverged on (3) when initialized with a single-number random seed. Therefore, we believe that systems biology software would benefit from a PRNG ontology that supports points (1), (2), and (3) above. If two implementations are identified in the ontology, and have identical parameterization and initial conditions according to the ontology, then the random number sequences produced by both implementations should be identical. We believe this criterion should be tested on each new algorithm or implementation prior to adding it to the ontology. The ontology would thus be self-correcting because %discrepancies would point to problems in either the new implementation or existing implementations in the ontology.
PRNGs would be validated by consensus, similar to other approaches for simulators \cite{bergmann2008comparing}.

\subsection{Special Considerations for Multi-algorithm WC Models}
Multi-algorithm WC models strive to represent every gene and cell function by combining multiple submodels of individual cellular pathways, each represented using different mathematics and each trained using different experimental data \cite{Karr2015, macklin2014future, carrera2015build}. By way of illustration, consider a simple model comprising a genetic network which regulates a metabolic pathway. Assume that gene expression data is available but kinetic parameters for enzymes in the metabolic pathway are unknown. Thus, we can model the genetic network using ODEs in order to capture differences in promoter activity. We can use flux balance analysis for the metabolic network since kinetic data is unavailable. This multi-algorithm methodology is motivated by the desire to model biological systems as completely as possible, given our current knowledge, by simultaneously using fine-grained representations and coarse-grained representations to represent well- and poorly-characterized pathways, respectively.
...
Multi-threaded and distributed computing are two important methods for accelerating the simulation of computationally expensive models such as WC models. Parallel programs use multiple threads and/or cores to simultaneously execute multiple computations. The order in which these threads and/or cores complete computations depends on the operating environment, and the order in which computations complete can affect subsequent computations and, in turn, simulation results. WC models also face these problems. Consider a simple example of a WC model which binds proteins to the chromosome. At each time step, the model attempts to bind a given protein to a given locus. If the locus is a promoter, it will have a high amount of traffic and it may happen that two proteins attempt to bind in a given time step. In computer science, this is called a race condition. It can be fixed using synchronization (shown in the pseudo-code below) so that only one protein may bind. However, synchronization does not by itself ensure that the same protein will bind for different runs. This is not due only to the use of random numbers --- it is due to the fact that the CPU scheduler does not guarantee repeatable scheduling.

\begin{Verbatim}[fontsize=\small]
# process proteins which can bind
parallel for protein_to_bind in proteins {
  # randomly bind proteins
  if chance_to_bind() > threshold {
    # select locus on chromosome to bind at
    # (weighted toward e.g. promoters)
    locus = select_weighted_random_locus();
    # acquire a mutex on the locus
    # only one protein can bind
    try_lock(locus);
    # if another protein isn't already bound
    if is_locked(locus) {
      # bind the protein
      bind_protein_at_locus(locus);
    }
  }
}
\end{Verbatim}

Other sources of non-determinism in parallel computing include memory layout, delays caused by devices and processes, and user input. Thus, parallel programs can generate non-deterministic simulation results due to variable operating environments. For example, if two WC submodels which are executed in parallel attempt to bind proteins to the same site on the chromosome, the submodel which completes its computations first will bind the protein to the chromosome. However, when the simulation is re-run under a different load on the computer, the other submodel may complete its computations first, resulting in the binding of a different protein the site. This small difference can lead to further differences between later time steps, resulting in different simulation results solely due to different operating environments.
...
\begin{figure*}
  ...
  \begin{subfigure}[b]{0.5\textwidth}
    ...
    \begin{array}{c c c}
      ...
      \left( 1 \right) & \varnothing \to x & \frac{1}{2} + V_m \frac{x^n}{10^6 ~\SI{}{\micro\Molar} + x^n} \\ \\
      \left( 2 \right) & x \to \varnothing & \frac{k}{x} \\ \\
    \end{array}
    \]
    Where:
    \[
      x = 24 ~\SI{}{\micro\Molar} ~ \textrm{at} ~ t = 0,
    \]
    \[
      n = 4,
    \]
...
The inclusion of unit information in systems biology models is often overlooked. However, unit information is particularly important for integrating hierarchical models, where it can prevent costly human error due to using incorrect units. %Indeed, the Mars Climate Orbiter infamously disintigrated in the Martian atmosphere as a result of inconsistent unit use between two different software systems.
In the case of different submodels being produced by different teams, unit information provides one type of validation that the models will be compatible. Systems biology simulators and tools should check for compatible units before integrating two different submodels, and issue an error to the user if unit-consistent integration is not possible.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{photos/goldberg.jpg}}]{Arthur P. Goldberg}
