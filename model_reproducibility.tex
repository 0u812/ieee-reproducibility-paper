...
Furthermore, our experience is that different implementations of a given PRNG tend to diverge in subtle ways. Thus, we recommend that developers of PRNGs document any non-standard operations present in their implementations, and users follow this documentation to avoid any potential discrepancies in results caused by implementation idiosyncrasies.
...
C{\small ORE}D{\small ET} and D{\small THREADS} have both been shown to impose minimal overhead \cite{liu2011dthreads}. We recommend that simulation software developers strive to implement an option for deterministic, repeatable simulations, and we offer C{\small ORE}D{\small ET} and D{\small THREADS} as examples of how this can be achieved in synchronization of processes.
...
Whether or not these tools and approaches should be used in a given software package depends on whether the benefits of repeatable simulations outweigh the cost of implementing deterministic synchronization. We anticipate that this balance will shift more toward repeatability in future hybrid simulation platforms as the burden of checking, debugging, and validating results increases dramatically with the size and complexity of the model. Implementing repeatable simulations shifts the burden from the modeler to the software developer. If a simulation environment can be reused in multiple modeling studies, we deem this tradeoff worthwhile.

\subsection{Special Considerations for Chaotic Systems}

Special care should also be taken to repeat the simulation results of chaotic models which are highly sensitive to their initial conditions. The separation rate of trajectories of a model can be quantified (e.g. Lyapunov exponents) and gives a measure of the model's predictability \cite{nehmzow2008robot}. Models with low predictability tend to diverge rapidly and statistical tests become unsuitable for validation. However, in principle, these issues could be addressed by a formal treatment, such as \cite{kocarev1996generalized}. We have chosen not to focus on chaotic modeling in this discussion of reproducibility because we believe that (1) it is considerably more difficult to implement than the non-chaotic case and (2) in our experience, chaos does not tend dominate single-cell behavior on the time scale of a single cell cycle. Nevertheless, we note that high precision should be used in encoding initial conditions and parameters so that chaotic model simulation results can be repeated.

\section{Methods for Verifying Repeatability}
After adopting the methods outlined above to repeatably simulate models, we recommend that systems biology modelers and software developers verify repeatability by testing the equivalence (either by direct comparison or statistical comparison) of simulation results generated by multiple simulation software programs for the same model, and then correcting any discrepancies identified by this analysis. Bergmann and Sauro \cite{bergmann2008comparing} and Evans et al. \cite{evans2008sbml} have used this approach to assess the repeatability of models across several simulators, identifying errors in several simulation programs.

Model checking and unit testing are two formalisms that can be used to verify repeatability. Simulation-based model checkers, such as SPIN \cite{holzmann1997model}, are extensively used in electrical engineering to verify the behavior of models. Simulation-based checkers execute multiple simulations and then verify that the results are consistent with the user-supplied specification of the model's behavior \cite{kwiatkowska2011prism}. Model verification systems could be adopted to verify the repeatability of systems biology models. Unit testing is a powerful strategy for verifying the behavior of software. Unit testing could also be used to verify the repeatability of systems biology models and simulation programs by (1) using multiple simulators to execute multiple simulations and (2) verifying that their predictions are consistent with known requirements (e.g. logical constraints such as energy balance or statistical tests as described below).
