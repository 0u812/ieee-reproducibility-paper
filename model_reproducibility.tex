...
Special care should also be taken to repeat the simulation results of chaotic models which are highly sensitive to their initial conditions. Adopting any of a number of measures of predictability (e.g. Lyapunov exponents) allows us to estimate the time to divergence \cite{fraedrich1987estimating}, which in turn yields the time range over which the model's predictions are useful \cite{nehmzow2008robot}. This can also limit the applicability of statistical tests. These issues are inherent in chaotic models, which require specialized treatment for testing and validation. We note that high precision should be used in encoding initial conditions and parameters so that chaotic model simulation results can be repeated.
