...
The above example shows that \textit{reproducibility} applies to a process with a human in the loop. Different researchers are unlikely to arrive at the same numerical results, and tracking down discrepancies often helps illuminate sources of error. By contrast, machines can, in principle, replicate a numerical result exactly without human intervetion. \textit{Replicability} is a technological feature, whereas \textit{reproducibility} is a property of the research process. Consider again the above example and suppose Bob's simulation was based on a stochastic method instead of a deterministic one. Alice can still compare the propensities at the initial time point, but if the initial propensities are identical then the timecourse behavior of the model will not help her to isolate the discrepancy.

Replicability is important when the simulation results must be recreated exactly. There are certainly times when this requirement does not apply, but since our discussion here is based on the outcome of a workshop to re-encode a whole-cell model of \textit{Mycoplasma genitalium} \cite{Karr2012}, we will focus on the role of exactly repeatable numerical simulations and the role they play in the development, encoding, and distribution of complex hybrid models.
